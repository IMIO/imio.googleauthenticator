...
Commit with `git commit --no-verify`.
  ...
  <verify>
    <automated>grep -q 'src/imio/googleauthenticator' .coveragerc && grep -q 'src/imio.googleauthenticator.egg-info' cleanup.sh && test ! -f .hgignore && test ! -f .hg.packed && make help | grep -q purge && grep -q 'imio.googleauthenticator' docs/conf.py && grep -q 318 CLAUDE.md && grep -q 318 .planning/STATE.md && test -z "$(git grep -i collective -- setup.py MANIFEST.in .coveragerc cleanup.sh | grep -v 'collective\.\(recipe\|upgrade\|z3cform\|profiler\)' | grep -v 'buildout\.plonetest')" && bin/test -t '!robot'</automated>
  </verify>
  <acceptance_criteria>
