...
Commit with `git commit --no-verify`.
  ...
  <verify>
    <automated>grep -q "meta_type = 'iMio Google Authenticator PAS'" src/imio/googleauthenticator/pas_plugin.py && grep -q "PAS_ID = 'google_auth'" src/imio/googleauthenticator/setuphandlers.py && ! grep -q collective src/imio/googleauthenticator/setuphandlers.py && ! grep -q collective src/imio/googleauthenticator/www/add_google_authenticator_form.zpt && test "$(git show --stat --name-only --format= HEAD | grep -c .)" -le 5 && bin/test -t '!robot'</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `www/add_google_authenticator_form.zpt` names the new `meta_type` and contains no occurrence of the old one.
    - `README.rst` and `docs/index.rst` quote the new plugin title in their installation step.
    - `git show --stat HEAD` lists at most 5 files, all of them from this task's file list — the commit is isolated.
    - `bin/test -t '!robot'` exits 0 with 12 tests, 0 failures, 0 errors; in particular the layer setup completes, which is what proves exactly one `meta_type` is registered.
  </acceptance_criteria>
