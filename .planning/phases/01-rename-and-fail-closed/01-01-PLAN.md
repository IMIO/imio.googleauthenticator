...
<task type="tracer">
  ...
  <precondition>`git status --porcelain src/` is empty (the purge step is `git clean -xdf src/`, so untracked work *inside `src/`* would be lost; untracked files elsewhere — `.claude/**`, `.planning/**` — are out of the clean's reach and are expected to be present), `bin/buildout` exists, and `var/filestorage/` contains no `Data.fs`.</precondition>
...
and 2 the shell assertions written out below, commit 3 the `<verify>` block's full command.
